...
public:
    RobotArm() {
        armMesh = std::make_unique<Mesh>();
        jointMesh = std::make_unique<Mesh>();
        endEffectorMesh = std::make_unique<Mesh>();
    }
...
private:
    void renderFirstArm(Shader& shader) {
        ...
        glm::mat4 modelArm1 = glm::mat4(1.0f);
        modelArm1 = glm::translate(modelArm1, glm::vec3(0.5f, 0.0f, -0.5f)); 
        ...
        shader.setMat4("model", glm::value_ptr(modelArm1));
    ...
    void renderJoint(Shader& shader) {
        // 2. Articular Cube 
        glm::mat4 modelJoint = glm::mat4(1.0f);
        modelJoint = glm::translate(modelJoint, glm::vec3(0.5f, 0.0f, -0.5f));
        ...
        modelJoint = glm::scale(modelJoint, glm::vec3(0.15f, 0.15f, 0.15f));

        shader.setMat4("model", glm::value_ptr(modelJoint));
        shader.setVec3("objectColor", glm::vec3(0.5f, 0.0f, 0.5f)); 
        jointMesh->draw();
    }
    ...
    void renderSecondArm(Shader& shader) {
        ...
        shader.setMat4("model", glm::value_ptr(modelArm2));
        shader.setVec3("objectColor", glm::vec3(0.6f, 0.6f, 0.6f)); 
        armMesh->draw();
    }
    ...
    void renderEndEffector(Shader& shader) {
        ...
        modelEnd = glm::translate(modelEnd, glm::vec3(0.0f, 0.5f + 0.15f, 0.0f)); 
        modelEnd = glm::rotate(modelEnd, glm::radians(5.0f), glm::vec3(1.0f, 0.0f, 0.0f));
        modelEnd = glm::rotate(modelEnd, (float)glfwGetTime(), glm::vec3(0.0f, 1.0f, 0.0f));
        modelEnd = glm::scale(modelEnd, glm::vec3(0.3f, 0.3f, 0.3f));

        shader.setMat4("model", glm::value_ptr(modelEnd));
